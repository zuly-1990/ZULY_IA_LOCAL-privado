...
## Control de Calidad Humano (Visto Bueno)

Para evitar la "contaminación" del conocimiento de Zuly con errores, se ha implementado un sistema de **Visto Bueno**. Zuly no aprenderá nada de forma permanente sin tu aprobación explícita.

### Flujo de Aprendizaje Seguro

1.  **Ejecución:** Realizas una petición (ej: `zuly execute "crea un cubo"`).
2.  **Registro Temporal:** Zuly guarda la lógica exitosa en una "Sala de Espera" (PENDING).
3.  **Revisión:** Puedes ver qué hay pendiente con `zuly review`.
4.  **Aprobación:** Si el resultado en Blender es correcto, das el visto bueno: `zuly approve <id>`.
5.  **Descarte:** Si el resultado no te gusta, lo borras: `zuly reject <id>`.

### Comandos de Gestión

| Comando | Descripción |
| :--- | :--- |
| `zuly review` | Muestra la lista de patrones esperando tu aprobación. |
| `zuly approve <id>` | Aprueba el patrón y lo mueve a la memoria de entrenamiento (`STAGING`). |
| `zuly reject <id>` | Borra el patrón pendiente definitivamente. |

> [!IMPORTANT]
> **Integridad del Sistema:** Este paso manual es lo que garantiza que Zuly mantenga un nivel de calidad profesional (9.9/10). Sin tu aprobación, el conocimiento no se cristaliza.
