#!/usr/bin/env python3
"""
test_blender_advanced.py - Pruebas avanzadas de LYZU en Blender

Ejecutar dentro de Blender:
    blender --background --python test_blender_advanced.py

O manualmente en Blender (Scripting tab):
    exec(open('test_blender_advanced.py').read())
"""
...
# Import LYZU
from core.lyzu_core import LYZUCore, ContextualMemory
...
def test_advanced_handlers():
    ...
    try:
        memory = ContextualMemory(max_turns=500)
        lyzu = LYZUCore(mode="reactive", memory=memory)
        print(f"✅ LYZU inicializado en modo REACTIVE")
        print(f"   - Max turnos: {memory.max_turns}")
    ...
    try:
        ...
        for obj in bpy.data.objects:
            if obj.name == "CubeAdvanced":
                ...
                print(f"   - Nueva rotación: {obj.rotation_euler[:]}")
                print(f"   - Nueva escala: {obj.scale[:]}")
                break
    ...
    try:
        ...
        print(f"   - Turnos registrados: {stats['total_turns']}")
        print(f"   - Turnos archivados: {stats['archived_turns']}")
        print(f"   - Uso de memoria: {stats['memory_usage_percentage']:.1f}%")
        print(f"   - Max turnos: {stats['max_turns']}")
    ...
    try:
        handlers = lyzu.intent_router.handlers
    ...
    print(f"  - Turnos en memoria: {lyzu.memory.current_turn_count}")
