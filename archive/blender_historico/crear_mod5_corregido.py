...
# -*- coding: utf-8 -*-
"""
MOD-005: Estacion Espacial - CORREGIDO (paneles con espesor)
"""
...
print("="*70)
print("MOD-005: Estacion Espacial (CORREGIDO)")
print("="*70)
...
print("\\n" + "="*70)
print("CONSTRUYENDO ESTACION ESPACIAL - CORREGIDA")
print("="*70)
...
# CONECTORES - Usar cubos en lugar de cilindros para evitar N-Gons
print("Creando conectores (cubos para evitar N-Gons)...")
conectores_pos = [
    ((1.5, 0, 0), (0, 0, 1.57)), ((-1.5, 0, 0), (0, 0, 1.57)),
    ((0, 1.5, 0), (0, 0, 0)), ((0, -1.5, 0), (0, 0, 0)),
]
for i, (loc, rot) in enumerate(conectores_pos):
    bpy.ops.mesh.primitive_cube_add(size=1, location=loc)
    conn = bpy.context.active_object
    conn.name = "Conector_" + str(i+1)
    conn.scale = (0.3, 0.3, 1.5)
    conn.rotation_euler = rot
    bpy.ops.object.transform_apply(scale=True, rotation=True)
print("  [OK] Conectores (cubos)")

# PANELES SOLARES - Cubos delgados en lugar de planos (evita non-manifold)
print("Creando paneles solares (con espesor)...")
for i, (x, y) in enumerate([(4, 0), (-4, 0), (0, 4), (0, -4)]):
    ...
    panel.scale = (2, 1, 0.02)  # Espesor 0.02m
    panel.rotation_euler = (0, 0.5, 0)
    bpy.ops.object.transform_apply(scale=True, rotation=True)
print("  [OK] Paneles con espesor")
...
print("\\n[OK] Guardado: " + output)
print("[OK] MOD-005 CORREGIDO - paneles con espesor, conectores sin N-Gons")
'''

script_path = zuly_path / 'temp_mod5_corregido.py'
...
script_path.unlink()
print("\n[OK] MOD-005 corregido y recreado")
