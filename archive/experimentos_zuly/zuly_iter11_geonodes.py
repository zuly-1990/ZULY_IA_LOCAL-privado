...
BLEND_OUT = '/opt/zuly/Zuly_Villa_Savoye_Nodos_Iter_11.blend'
...
def create_master_tree(mat_ladrillo, mat_marcos, mat_concreto):
    ...
    f_switch = tree.nodes.new('NodeFrame'); f_switch.name = "SELECTOR BIM OUTLINER (ROBUST)"; f_switch.label = "SELECTOR BIM OUTLINER (ROBUST)"; f_switch.use_custom_color = True; f_switch.color = (0.1, 0.6, 0.3)
    
    comp1 = tree.nodes.new('FunctionNodeCompare'); comp1.data_type = 'INT'; comp1.operation = 'EQUAL'; comp1.inputs[3].default_value = 1; comp1.location = (1000, 400); comp1.parent = f_switch
    comp2 = tree.nodes.new('FunctionNodeCompare'); comp2.data_type = 'INT'; comp2.operation = 'EQUAL'; comp2.inputs[3].default_value = 2; comp2.location = (1000, 200); comp2.parent = f_switch
    comp3 = tree.nodes.new('FunctionNodeCompare'); comp3.data_type = 'INT'; comp3.operation = 'EQUAL'; comp3.inputs[3].default_value = 3; comp3.location = (1000, 0); comp3.parent = f_switch
    
    # Enlazar entrada de INT a comp (in_node.outputs[1] = Piso_ID, comp.inputs[2] = INT A)
    tree.links.new(in_node.outputs[1], comp1.inputs[2])
    tree.links.new(in_node.outputs[1], comp2.inputs[2])
    tree.links.new(in_node.outputs[1], comp3.inputs[2])
    
    # NUEVO: Usar Separate Geometry (MÁS ROBUSTO QUE SWITCH)
    sep1 = tree.nodes.new('GeometryNodeSeparateGeometry'); sep1.location = (1300, 400); sep1.parent = f_switch
    sep2 = tree.nodes.new('GeometryNodeSeparateGeometry'); sep2.location = (1300, 200); sep2.parent = f_switch
    sep3 = tree.nodes.new('GeometryNodeSeparateGeometry'); sep3.location = (1300, 0); sep3.parent = f_switch
    
    # Geometry In
    tree.links.new(join_p1.outputs[0], sep1.inputs['Geometry'])
    tree.links.new(out_muros_2, sep2.inputs['Geometry'])
    tree.links.new(join_p3.outputs[0], sep3.inputs['Geometry'])
    
    # Selection In
    tree.links.new(comp1.outputs[0], sep1.inputs['Selection'])
    tree.links.new(comp2.outputs[0], sep2.inputs['Selection'])
    tree.links.new(comp3.outputs[0], sep3.inputs['Selection'])
    
    # Join Final
    join_final = tree.nodes.new('GeometryNodeJoinGeometry'); join_final.location = (1600, 200); join_final.parent = f_switch
    tree.links.new(sep1.outputs['Selection'], join_final.inputs[0])
    tree.links.new(sep2.outputs['Selection'], join_final.inputs[0])
    tree.links.new(sep3.outputs['Selection'], join_final.inputs[0])
    
    # Output
    tree.links.new(join_final.outputs[0], out_node.inputs[0])
    
    return tree

def build_iter_11():
    print("[ZULY] Construyendo Iteración 11 (Refactorización a Separate Geometry)...")
    ...
    eval_p1 = obj_p1.evaluated_get(depsgraph)
    print(f"VERIFICACION VERTICES PISO 1: {len(eval_p1.data.vertices)}")
    eval_p2 = obj_p2.evaluated_get(depsgraph)
    print(f"VERIFICACION VERTICES PISO 2: {len(eval_p2.data.vertices)}")
    
    print(f"✅ Iteración 11 Completada: {BLEND_OUT}")
