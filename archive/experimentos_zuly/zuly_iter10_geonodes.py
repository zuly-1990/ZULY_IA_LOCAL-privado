...
BLEND_OUT = '/opt/zuly/Zuly_Villa_Savoye_Nodos_Iter_10.blend'
...
def create_master_tree(mat_ladrillo, mat_marcos, mat_concreto):
    ...
    # IMPORTANTE: En Blender 3.6 debemos usar índices explícitos absolutos.
    # [2] y [3] son los sockets de tipo INT en el nodo Compare
    ...
    # Sockets Switch: [14]=False(GEOMETRY), [15]=True(GEOMETRY), [0]=Switch(BOOLEAN)
    sw1 = tree.nodes.new('GeometryNodeSwitch'); sw1.input_type = 'GEOMETRY'; sw1.location = (1300, 400); sw1.parent = f_switch
    sw2 = tree.nodes.new('GeometryNodeSwitch'); sw2.input_type = 'GEOMETRY'; sw2.location = (1500, 200); sw2.parent = f_switch
    sw3 = tree.nodes.new('GeometryNodeSwitch'); sw3.input_type = 'GEOMETRY'; sw3.location = (1700, 0); sw3.parent = f_switch
    
    # Enlaces de Compare usando in_node.outputs[1] (que es Piso_ID)
    ...
    # Enlaces de los Switches
    tree.links.new(comp3.outputs[0], sw3.inputs[0])
    tree.links.new(join_p3.outputs[0], sw3.inputs[15])
    
    tree.links.new(comp2.outputs[0], sw2.inputs[0])
    tree.links.new(sw3.outputs[6], sw2.inputs[14]) # outputs[6] es GEOMETRY
    tree.links.new(out_muros_2, sw2.inputs[15])
    
    tree.links.new(comp1.outputs[0], sw1.inputs[0])
    tree.links.new(sw2.outputs[6], sw1.inputs[14])
    tree.links.new(join_p1.outputs[0], sw1.inputs[15])
    
    # Salida Final (Geometry = inputs[0])
    tree.links.new(sw1.outputs[6], out_node.inputs[0])
    
    return tree

def build_iter_10():
    print("[ZULY] Construyendo Iteración 10 (Auditoria de Sockets Absolutos)...")
    ...
    print(f"✅ Iteración 10 Completada: {BLEND_OUT}")
