...
BLEND_OUT = '/opt/zuly/Zuly_Villa_Savoye_Nodos_Iter_6.blend'

# PARÁMETROS CORREGIDOS POR REVISIÓN PROFUNDA BBOX
RADIO_PILOTE = 0.15
ALTO_PILOTE = 3.3      # Piso 1 va desde Z=-0.15 hasta Z=3.15 (Total: 3.3m)
Z_PILOTE = 1.5         # Centro de la columna (-0.15 + 3.15)/2
GRID_X = 14.7  
GRID_Y = 16.2  

Z_CUBIERTA = 9.8       # El Piso 3 llega hasta Z=9.70. El techo debe ir encima de eso.
...
def create_muros_tree(name, source_objects, material, frame_name, frame_color):
    ...
    frame = tree.nodes.new('NodeFrame'); frame.name = frame_name; frame.label = frame_name; frame.use_custom_color = True; frame.color = frame_color
    join = tree.nodes.new('GeometryNodeJoinGeometry'); join.location = (0, 0); join.parent = frame
    weld = tree.nodes.new('GeometryNodeMergeByDistance'); weld.location = (200, 0); weld.inputs['Distance'].default_value = 0.005; weld.parent = frame
    extrude = tree.nodes.new('GeometryNodeExtrudeMesh'); extrude.location = (400, 0); extrude.inputs['Offset Scale'].default_value = 0.02; extrude.parent = frame
    mat_node = tree.nodes.new('GeometryNodeSetMaterial'); mat_node.location = (600, 0); mat_node.inputs['Material'].default_value = material; mat_node.parent = frame
...
def build_iter_6():
    print("[ZULY] Construyendo Iteración 6 (Corrección de Alturas)...")
    ...
    # Piso 1
    obj_p1 = bpy.data.objects.new("Muros_Piso_1", bpy.data.meshes.new("Mesh_Piso1"))
    ...
    frame_p = t_pilotes.nodes.new('NodeFrame'); frame_p.name = "CARPETA: PILOTES"; frame_p.label = "CARPETA: PILOTES"; frame_p.use_custom_color = True; frame_p.color = (0.2, 0.5, 0.8)
    ...
    trans_c.inputs['Translation'].default_value = (9.8, 10.8, Z_CUBIERTA)
    ...
    print(f"✅ Iteración 6 Completada: {BLEND_OUT}")
